app.controller("ActivitiesController",["$scope","Activity","$window",function(t,e,i){function r(t){var e=new Date;try{new Date(t.ended)<e?t.status="end":new Date(t.started)>e?t.status="nos":t.status="ing"}catch(i){}return t}t.activityFilter=["全部","进行中","未开始","已结束"],t.choice="全部",i.pull=!0,t.activityItems=[],t.filterBar="ng-hide";var a=0,n=0;t.changeFilter=function(){t.choice=this.filter},angular.element(i).bind("scroll",function(e){var r=e.target.body;r.scrollHeight-r.clientHeight-r.scrollTop<600&&i.pull&&!t.last?(t.getActivities(),i.pull=!1):r.scrollHeight-r.clientHeight-r.scrollTop>600&&(i.pull=!0)}),t.$on("$destroy",function(e,r){angular.element(i).unbind("scroll"),t.activityItems=void 0}),t.formFilter=function(){t.query={actType:"form"},a=0,t.last=!1,t.activityItems=[],t.getActivities(),t.typeHide=!0,t.type="表单"},t.voteFilter=function(){t.query={actType:"vote"},a=0,t.last=!1,t.activityItems=[],t.getActivities(),t.typeHide=!0,t.type="投票"},t.seckillFilter=function(){t.query={actType:"seckill"},a=0,t.last=!1,t.activityItems=[],t.getActivities(),t.typeHide=!0,t.type="抢票"},t.allTypeFilter=function(){t.query=void 0,t.typeHide=!0,t.type="类型"},t.allStatusFilter=function(){t.query=void 0,t.status="状态",t.statusHide=!0},t.ingFilter=function(){t.query={status:"ing"},t.statusHide=!0,n=1,a=0,t.last=!1,t.activityItems=[],t.getActivities(),t.status="进行中"},t.nosFilter=function(){t.query={status:"nos"},t.statusHide=!0,n=2,a=0,t.last=!1,t.activityItems=[],t.getActivities(),t.status="未开始"},t.endFilter=function(){t.query={status:"end"},t.statusHide=!0,n=-1,a=0,t.last=!1,t.activityItems=[],t.getActivities(),t.status="已结束"},t.titleFilter=function(){filterTitle?t.query={title:filterTitle}:t.query=void 0},t.getActivities=function(){if(!t.$currentUser)return void t.$emit("auth:loginRequired");var i=t.query||{},s=i.actType||"all";e.getMySchoolActiveties({school:t.$currentUser.school,actType:s,status:n,page:a},function(e){for(x in e)"activties"===x&&(a++,(e[x].length<16||0===e[x].length)&&(t.last=!0),e[x].forEach(function(e){t.activityItems.push(r(e))}))})},t.$currentUser?t.getActivities():e.find(function(e){t.activityItems=e,t.activityItems.forEach(function(t){r(t)})})}]),app.controller("ActivityController",["$scope","Activity","User","$stateParams","$interval","uploadFile","$location","Article",function(t,e,i,r,a,n,s,o){function l(t){var e=t%60,i=Math.floor(t/86400),r=Math.floor((t-3600*i*24)/3600),a=Math.floor((t-3600*i*24-3600*r)/60);return{d:i,h:r,m:a,s:e}}function c(){u=a(function(){e.prototype_get_seckills({id:r.id},function(e){t.seckill=e[0]})},1e3)}var u,v;e.findById({id:r.id},function(i){t.activity=i,t.article=i,o.prototype_get_comments({id:i.id},function(e){t.article.comments=e,t.article.showCommentBox=!0}),"common"!==i.actType&&e["prototype_get_"+i.actType+"s"]({id:r.id},function(e){if(t[i.actType]=e[0],"seckill"===i.actType){var r=Math.floor((new Date(t.seckill.started)-new Date(t.seckill.serverTime))/1e3),n=l(r);t.countDown=r,t.seckillTime=n,v=a(function(){r--;var e=r%60,i=Math.floor(r/86400),n=Math.floor((r-3600*i*24)/3600),s=Math.floor((r-3600*i*24-3600*n)/60);t.seckillTime={d:i,h:n,m:s,s:e},0>=r&&a.cancel(v)},1e3)}})}),t.$on("$destroy",function(){a.cancel(v),a.cancel(u)}),t.onClickJoinActivity=function(){var e=t.activity.actType;t.activityEnded=!1,t.$currentUser||t.$emit("auth:loginRequired"),t.$currentUser&&"学号"===t.activity.verifyRule?t.activity.verifyId=t.$currentUser.studentId:t.$currentUser&&"手机"===t.activity.verifyRule&&(t.activity.verifyId=t.$currentUser.phone),"common"!==e&&(t.result=new Array(t[e]["_"+e+"Items"].length)),new Date-new Date(t.activity.ended)>0&&(t.activityEnded=!0)},t.checkedForVote=function(){var e=t.result.filter(function(t){return t});return t.vote.max&&e.length>t.vote.max?(this.result[this.$index]=void 0,void Materialize.toast("只能选择"+t.vote.max+"项哦",2e3)):void(this.result[this.$index]?this.voteIteam.count++:this.voteIteam.count--)},t.submitFormResult=function(){if(!t.$currentUser)return t.$emit("auth:loginRequired");if(t.activity.verifyRule&&!t.activity.verifyId)return void Materialize.toast("验证规则必须填",2e3);for(var e in t.form._formItems)"select"===t.form._formItems[e].type?(t.result[e].type="selelct",t.result[e].name=t.result[e].option,t.result[e].option=void 0):t.result[e].type=t.form._formItems[e].type;var r={verifyId:t.activity.verifyId,formId:t.form.id,created:new Date,result:t.result};i.prototype_create_formResults({id:t.$currentUser.id},r,function(t){1e3===t.status||1100===t.status?Materialize.toast(t.message,2e3):Materialize.toast("参与成功,可在个人主页查看结果",4e3)})},t.uploadFile=function(){if(!t.$currentUser)return t.$emit("auth:loginRequired");var e=this.$parent;t.result[e.$index]={};var i=document.getElementById("file").files[0];if(i){var r=t.tem;n.file(i,"user",t.$currentUser.id).success(function(i){t.result[e.$index].name=r,t.result[e.$index].url=i.url})}},t.submitVoteResult=function(){var e=[];if(!t.$currentUser)return t.$emit("auth:loginRequired");if(t.activity.verifyRule&&!t.activity.verifyId)return void Materialize.toast("验证规则必须填",2e3);for(var r in t.result)t.result[r]&&e.push(t.vote._voteItems[r].id);if(t.vote.limit&&e.length<t.vote.limit)return void Materialize.toast("必须选择"+t.vote.limit+"项哦",2e3);if(t.vote.max&&e.length>t.vote.max)return void Materialize.toast("只能选择"+t.vote.max+"项哦",2e3);if(e.length<=0)return void Materialize.toast("至少选择一项哦",2e3);var a={verifyId:t.activity.verifyId,voteId:t.vote.id,created:new Date,result:e};i.prototype_create_voteResults({id:t.$currentUser.id},a,function(t){1e3===t.status?Materialize.toast(t.message,2e3):Materialize.toast("参与成功,可在个人主页查看结果",4e3)})},t.submitSeckillResult=function(){if(c(),!t.$currentUser)return t.$emit("auth:loginRequired");if(t.activity.verifyRule&&!t.activity.verifyId)return void Materialize.toast("验证规则必须填",2e3);var e={created:new Date,verifyId:t.activity.verifyId,itemId:this.seckillItem.id,seckillId:t.seckill.id};i.prototype_create_seckillResults({id:t.$currentUser.id},e,function(t){1e3===t.status||1100===t.status?Materialize.toast(t.message,2e3):Materialize.toast("参与成功,可在个人主页查看结果",4e3)})}}]);
